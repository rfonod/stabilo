...
- **Robust estimation**: MAGSAC++ by default, plus GC-RANSAC, three LO-RANSAC (USAC) variants, PROSAC, RHO, LMEDS, RANSAC (`--ransac-method 36`), with tunable threshold, iterations, and confidence (`--ransac-epipolar-threshold 1.5`). The threshold is measured in full-resolution pixels by default, or in downsampled-image pixels with `--ransac-threshold-space processed`. See [RANSAC methods](docs/usage.md#11-transformation-types-and-ransac-methods).
